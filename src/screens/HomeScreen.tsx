...
export function HomeScreen({ navigation }: Props) {
  ...
  const moveUp = async (index: number, list: ScheduledVideo[]) => {
    ...
    // Rebuild full array preserving app triggers at end
  ...
  // Upcoming: first active future datetime video
  ...
  // Scheduled datetime videos (excluding upcoming)
        ...
        <View style={styles.upcomingInfo}>
          <View style={styles.upcomingInfoLeft}>
            ...
            <View style={styles.triggerRow}>
              ...
              <Text style={[styles.triggerText, { color: '#9898d6' }]} numberOfLines={1}>
                {triggerLabel}
              </Text>
            </View>
          </View>
          <TouchableOpacity
            onPress={() => handleToggleActive(video)}
            hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}
          >
            <Ionicons
              name={video.isActive ? 'pause-circle-outline' : 'play-circle-outline'}
              size={24} color="#9898d6"
            />
          </TouchableOpacity>
          <TouchableOpacity
            onPress={() => setDeleteId(video.id)}
            hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}
          >
      ...
      >
        ...
        <View style={styles.actions}>
          ...
          >
            <Ionicons
              name={video.isActive ? 'pause-circle-outline' : 'play-circle-outline'}
              size={26} color="#9898d6"
            />
          </TouchableOpacity>
          <View style={styles.reorderBtns}>
            <TouchableOpacity
              onPress={() => moveUp(index, scheduledDatetimeVideos)}
              hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}
            >
              <Ionicons name="chevron-up" size={16} color={index === 0 ? colors.border : colors.accent} />
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => moveDown(index, scheduledDatetimeVideos)}
              hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}
            >
              <Ionicons name="chevron-down" size={16}
                color={index === scheduledDatetimeVideos.length - 1 ? colors.border : colors.accent} />
            </TouchableOpacity>
          </View>
          <TouchableOpacity
            onPress={() => setDeleteId(video.id)}
  ...
  // ── App trigger card — no pause toggle for play-once ───────────────────────
  const AppCard = ({ video }: { video: ScheduledVideo }) => {
    ...
    return (
      ...
      >
        ...
        <View style={styles.info}>
          ...
          <View style={styles.triggerRow}>
            <Ionicons name="phone-portrait-outline" size={13}
              color={hasPlayed ? colors.textLight : colors.danger} />
        ...
        {/* Only show pause toggle for "always" app triggers, not play-once */}
        <View style={styles.actions}>
          {!isPlayOnce && (
            <TouchableOpacity
              onPress={() => handleToggleActive(video)}
  ...
  const HowItWorks = () => (
    <View style={styles.howItWorks}>
      ...
      {[
        ...
        { n: '2', title: 'Set when to play', desc: 'Choose a specific date/time or link it to an app opening' },
...
const styles = StyleSheet.create({
  ...
  sectionTitle: {
    fontFamily: fonts.montserratBold, fontSize: 15,
  ...
  pausedBadge: {
    ...
    backgroundColor: '#9898d620', borderRadius: radius.sm,
  ...
  emptyContainer: {
    ...
    padding: spacing.xl, minHeight: 420,
  },
  emptyIcon: {
  ...
  emptyBody: {
    fontFamily: fonts.inter, fontSize: 13, color: colors.text,
  ...
  stepDesc: { fontFamily: fonts.inter, fontSize: 12, color: colors.textLight, marginTop: 3 },
